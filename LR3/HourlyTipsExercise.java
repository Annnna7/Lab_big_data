...
public class HourlyTipsExercise extends ExerciseBase {
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<Tuple3<Long, Long, Float>> hourlyMax = fares
				...
				// Суммируем чаевые для каждого водителя за час
				.process(new AddTips())
				// Группируем все результаты за один час
				.keyBy(tuple -> tuple.f0)  // f0 = timestamp окна
				.timeWindow(Time.hours(1))
				// Находим максимальную сумму в каждом часе
				.maxBy(2);  // 2 = индекс поля sumTips (третье поле)
	...
	/**
	 * Функция, которая агрегирует чаевые для одного водителя за часовое окно.
	 * На вход: TaxiFare (отдельная поездка)
	 * На выход: Tuple3<timestamp окна, ID водителя, сумма чаевых>
	 */
	public static class AddTips extends ProcessWindowFunction<
		...
		public void process(Long key, Context context, Iterable<TaxiFare> fares,
			...
			Float sumOfTips = 0F;
			// Суммируем все чаевые в этом окне для данного водителя
			for (TaxiFare fare : fares) {
				sumOfTips += fare.tip;
			}
			// Отправляем результат: (время окончания окна, ID водителя, сумма чаевых)
